...
Write-Host "Starting local API at http://127.0.0.1:8080/"
$api = Start-Process -FilePath $Python -ArgumentList @("webapp\scripts\local_api_server.py") -WorkingDirectory $Root -WindowStyle Hidden -PassThru
...
Write-Host "Close these processes from Task Manager or run: Stop-Process -Id $($watcher.Id),$($api.Id),$($server.Id)"
