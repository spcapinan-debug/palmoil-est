...
function farmMapPolygonParts(geometry = {}) {
...
function ringBounds(ring = []) {
  ...
  return xs.length && ys.length ? [Math.min(...xs), Math.min(...ys), Math.max(...xs), Math.max(...ys)] : [];
}
...
function polygonPartsConflict(left = [], right = []) {
  const leftRing = left[0] || [];
  const rightRing = right[0] || [];
  const leftBounds = ringBounds(leftRing);
  const rightBounds = ringBounds(rightRing);
  if (leftBounds.length !== 4 || rightBounds.length !== 4
      || leftBounds[2] <= rightBounds[0] || rightBounds[2] <= leftBounds[0]
      || leftBounds[3] <= rightBounds[1] || rightBounds[3] <= leftBounds[1]) return false;
  if (leftRing.slice(0, -1).some((point) => pointInRing(point, rightRing))
      || rightRing.slice(0, -1).some((point) => pointInRing(point, leftRing))) return true;
  for (let leftIndex = 0; leftIndex < leftRing.length - 1; leftIndex += 1) {
    for (let rightIndex = 0; rightIndex < rightRing.length - 1; rightIndex += 1) {
      if (properSegmentIntersection(leftRing[leftIndex], leftRing[leftIndex + 1], rightRing[rightIndex], rightRing[rightIndex + 1])) return true;
    }
  }
  return false;
}
...
function dedupeFarmMapFeatures(features = []) {
  ...
  let rawPolygonCount = 0;
  let dedupedPolygonCount = 0;
  ...
  for (const [, entries] of grouped) {
    ...
    const allParts = entries.flatMap(({ feature }) => farmMapPolygonParts(feature?.geometry));
    rawPolygonCount += allParts.length;
    const uniquePartByFingerprint = new Map();
    for (const part of allParts) uniquePartByFingerprint.set(geometryFingerprint({ type: "Polygon", coordinates: part }), part);
    const uniqueParts = [...uniquePartByFingerprint.values()];
    dedupedPolygonCount += allParts.length - uniqueParts.length;
    let geometryConflict = false;
    for (let left = 0; left < uniqueParts.length && !geometryConflict; left += 1) {
      for (let right = left + 1; right < uniqueParts.length; right += 1) {
        if (polygonPartsConflict(uniqueParts[left], uniqueParts[right])) {
          geometryConflict = true;
          break;
        }
      }
    }
    const resolution = geometryConflict ? "conflict"
      : uniqueParts.length > 1 ? "multi_part"
        : allParts.length > uniqueParts.length ? "deduped" : "single";
    if (resolution === "multi_part") multipartBlockCount += 1;
    if (entries.length > 1) {
      duplicateMapKeys.push({
        ...
        geometryConflict,
        resolution,
        polygonPartCount: uniqueParts.length,
        dedupedPolygonCount: allParts.length - uniqueParts.length,
      });
    }
    if (geometryConflict) {
    ...
    const geometry = geometryConflict || !uniqueParts.length ? null
      : uniqueParts.length === 1
        ? { type: "Polygon", coordinates: uniqueParts[0] }
        : { type: "MultiPolygon", coordinates: uniqueParts };
    uniqueFeatures.push({
      ...first.feature,
      properties: {
        ...
        polygon_part_count: uniqueParts.length,
      },
      geometry,
    });
  }
  ...
  return {
    ...
    dedupedPolygonCount,
    multipartBlockCount,
    unresolvedDuplicateCount: geometryConflicts.length,
    duplicateMapKeys,
    geometryConflicts,
...
function reconcileFarmAreaMap({ blocks = [], features = [], source = {} } = {}) {
  ...
  return {
    ...
    reconciliation: {
      ...
      rawPlacemarks: Number(source.placemarkCount ?? mapAudit.rawPlacemarkCount),
      polygonPlacemarks: Number(source.polygonPlacemarkCount ?? mapAudit.rawPlacemarkCount),
      pointPlacemarks: Number(source.pointPlacemarkCount || 0),
      otherPlacemarks: Number(source.otherPlacemarkCount || 0),
      rawPolygons: mapAudit.rawPolygonCount,
      ...
      dedupedPolygons: mapAudit.dedupedPolygonCount,
      multipartBlocks: mapAudit.multipartBlockCount,
      unresolvedDuplicates: mapAudit.unresolvedDuplicateCount,
