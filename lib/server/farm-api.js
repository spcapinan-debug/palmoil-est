...
function enforcePreviewDatabaseIsolation(url) {
  if (String(process.env.VERCEL_ENV || "").toLowerCase() !== "preview") return;
  const resolvedRef = supabaseProjectRef(url);
  const expectedRef = String(process.env.RC_STAGING_SUPABASE_REF || "").trim();
  const productionRef = String(process.env.RC_PRODUCTION_SUPABASE_REF || "").trim();
  if (!resolvedRef || !expectedRef || resolvedRef !== expectedRef
    || (productionRef && resolvedRef === productionRef)) {
    throw new ApiError(
      500,
      "RC_PREVIEW_PRODUCTION_DATABASE_FORBIDDEN",
      "RC_PREVIEW_PRODUCTION_DATABASE_FORBIDDEN"
    );
  }
}
...
function config() {
  ...
  enforcePreviewDatabaseIsolation(url);
...
const DATABASE_DOMAIN_ERRORS = new Map([
  ...
  ["PAYROLL_CANONICAL_RESULT_REQUIRED", 409],
  ["PAYROLL_VERIFIED_RESULT_REQUIRED", 409],
  ["PAYROLL_FROZEN_RATE_LINEAGE_MISMATCH", 409],
  ["PAYROLL_PERIOD_HALF_MONTH_REQUIRED", 409],
  ["PAYROLL_PERIOD_CLOSED_IMMUTABLE", 409],
  ["PAYROLL_TEAM_POOL_NOT_RECONCILED", 409],
  ["PAYROLL_ELIGIBLE_RESULT_NOT_PREPARED", 409],
  ["PAYROLL_APPROVED_QUALITY_RULE_REQUIRED", 409],
  ["CONTRACTOR_APPROVED_QUALITY_RULE_REQUIRED", 409],
...
module.exports = {
  ...
  clearAuthCookies,
  databaseDomainError,
  enforcePreviewDatabaseIsolation,
  config,
  errorResponse,
  json,
  optionalUuid,
