...
async function readBody(req, maxBytes = 2_000_000) {
  if (req.body != null) {
    if (Buffer.isBuffer(req.body)) {
      if (req.body.length > maxBytes) throw new ApiError(413, "PAYLOAD_TOO_LARGE", "Request body exceeds the allowed size");
      try {
    ...
    if (typeof req.body === "string") {
      if (Buffer.byteLength(req.body, "utf8") > maxBytes) throw new ApiError(413, "PAYLOAD_TOO_LARGE", "Request body exceeds the allowed size");
      try {
  ...
  for await (const chunk of req) {
    raw += chunk;
    if (Buffer.byteLength(raw, "utf8") > maxBytes) throw new ApiError(413, "PAYLOAD_TOO_LARGE", "Request body exceeds the allowed size");
  }
  if (!raw) return {};
  try {
