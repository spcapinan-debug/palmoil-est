...
function parseKml(kml, sourceFile = "doc.kml") {
  ...
  if (/<(?:\w+:)?NetworkLink\b|<(?:\w+:)?Model\b/i.test(xml)
      || /<(?:\w+:)?href\b[^>]*>\s*(?:https?|ftp|file):/i.test(xml)) {
  ...
  parser.on("opentag", (tag) => {
    ...
    if (local === "Placemark") placemark = { name: "", polygons: [] };
...
async function parseFarmAreaKmz(buffer) {
  ...
  const features = extracted.entries.flatMap((entry) => parseKml(entry.content, entry.fileName));
