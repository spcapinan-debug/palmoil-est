...
function renderAdvancedDashboard() {
  ...
  const latestInboundRows = records
    .filter(({ record }) => record.wpInOutType === "I")
    .sort((a, b) => String(b.record.weightDate || b.record.date || "").localeCompare(String(a.record.weightDate || a.record.date || "")))
    .slice(0, 6)
    .map(({ record, movement, scope }) => `
      <tr>
        <td>${displayDate(record.weightDate || record.date)}</td>
        <td>${record.wpDocNo || "-"}</td>
        <td>${record.wpCarLicense || "-"}</td>
        <td>${scope === "takuk" ? "ตะกุก" : "ปลายราง"}</td>
        <td class="left">${record.name || record.wpctCode || "-"}</td>
        <td class="num">${fmt(record.wpNetWeight)}</td>
        <td>${recordStandardBucket(record, movement)}</td>
        <td>${record.areaGroup || "-"}</td>
      </tr>`);

  const clearSummaryRows = yardRows.map((row) => `
    <tr>
      <td class="left"><span class="yard-badge ${row.scope === "takuk" ? "takuk" : "garden"}">${row.scope === "takuk" ? "T" : "E"}</span>${row.label}</td>
      <td class="num">${fmt(row.stats.opening)}</td>
      <td class="num">${fmt(row.stats.outbound)}</td>
      <td class="num">${fmt(row.stats.balance)}</td>
      <td class="num loss">${fmt(row.stats.loss)}</td>
      <td>ตรวจแล้ว</td>
  ...
  els.reportPage.innerHTML = `
    <div class="home-tabs">
      <button class="active" type="button">ภาพรวม</button>
      <button type="button" data-view="stock">Stock Report</button>
      <button type="button" data-view="clear">Clear Ramp Log</button>
    </div>
    <div class="home-dashboard">
      <section class="home-panel">
        <div class="home-panel-head">
          <h3>Stock Report สรุปตามลาน</h3>
          <button type="button" data-view="stock">ดูรายละเอียด</button>
        </div>
        ${simpleTable(["ลาน", "น้ำหนักรับเข้า", "น้ำหนักส่งออก", "น้ำหนักคงเหลือ", "น้ำหนักสูญหาย"], yardTableRows)}
      </section>
      <section class="home-panel">
        <div class="home-panel-head">
          <h3>Clear Ramp สรุปวันนี้</h3>
          <button type="button" data-view="clear">ดูรายละเอียด</button>
        </div>
        ${simpleTable(["ลาน", "ยกมา", "ส่งออก", "คงเหลือ", "สูญหาย", "สถานะ"], clearSummaryRows)}
      </section>
      <section class="home-panel wide">
        <div class="home-panel-head">
          <h3>ข้อมูลล่าสุด</h3>
          <div class="home-actions">
            <button type="button">ตัวกรอง</button>
            <button type="button" data-export-current="excel">Export Excel</button>
            <button type="button" data-export-current="pdf">Export PDF</button>
          </div>
        </div>
        ${simpleTable(["วันที่", "เลขที่ใบชั่ง", "ทะเบียนรถ", "ลานชั่ง", "ลูกค้า / เจ้าของ", "น้ำหนักสุทธิ", "RSPO", "AP Code"], latestInboundRows)}
      </section>
    </div>
    <div class="analytics-layout dashboard-depth">
